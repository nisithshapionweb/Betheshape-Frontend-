...
import AdminEditOtherPages from "./AdminEditOtherPages";
...
const AdminOtherPages = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => axiosPublic.put(`/other/${id}`, data),
    onSuccess: () => {
      Swal.fire("Updated!", "Other updated successfully", "success");
      queryClient.invalidateQueries(["Others"]);
    },
  ...
  const createMutation = useMutation({
    mutationFn: (newData) => axiosPublic.post("/other", newData),
    onSuccess: () => {
      queryClient.invalidateQueries(["Others"]);
      Swal.fire("✅ Success!", "Other  added successfully.", "success");
      resetForm();
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/other/${id}`),
    onSuccess: (res) => {
      if (res.data?.deletedCount > 0) {
        Swal.fire("Deleted!", "Other  deleted successfully.", "success");
      } else {
        Swal.fire("Info", "Other not found or already deleted.", "info");
      }
      queryClient.invalidateQueries(["Others"]);
    },
  ...
  return (
    <>
      <Helmet>
        <title>Admin | Other Management</title>
      </Helmet>

      <TittleAnimation
        tittle="Other "
        subtittle="Manage Other and Suggestion Fields"
      />
...
export default AdminOtherPages;
