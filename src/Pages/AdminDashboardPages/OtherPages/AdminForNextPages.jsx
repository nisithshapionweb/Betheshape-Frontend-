...
import AdminEditForNextPages from "./AdminEditForNextPages";

const AdminForNextPages = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => axiosPublic.put(`/other/fornext/${id}`, data),
    onSuccess: () => {
      Swal.fire("Updated!", "For Next updated successfully", "success");
      queryClient.invalidateQueries(["ForNext"]);
    },
  ...
  const createMutation = useMutation({
    mutationFn: (newData) => axiosPublic.post("/other/fornext", newData),
    onSuccess: () => {
      queryClient.invalidateQueries(["ForNext"]);
      Swal.fire("✅ Success!", "For Next  added successfully.", "success");
      resetForm();
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/other/fornext/${id}`),
    onSuccess: (res) => {
      if (res.data?.deletedCount > 0) {
        Swal.fire("Deleted!", "For Next  deleted successfully.", "success");
      } else {
        Swal.fire("Info", "For Next not found or already deleted.", "info");
      }
      queryClient.invalidateQueries(["ForNext"]);
    },
  ...
  return (
    <>
      <Helmet>
        <title>Admin | For Next Management</title>
      </Helmet>

      <TittleAnimation
        tittle="For Next "
        subtittle="Manage For Next and Suggestion Fields"
      />
...
export default AdminForNextPages;
