...
import OldGenarationFormatModal from "./OldGenarationFormatModal";

const AdminOldGenarationFormat = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (newData) =>
      axiosPublic.post("/seven-layer/oldGeneration", newData),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) =>
      axiosPublic.put(`/seven-layer/oldGeneration/${id}`, data),
    onSuccess: () => {
      Swal.fire(
        "✅ Updated!",
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/seven-layer/oldGeneration/${id}`),
    onSuccess: (res) => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(
        `/seven-layer/oldGenerationField/toggle`,
        {
          fieldName: "isActive",
...
export default AdminOldGenarationFormat;
