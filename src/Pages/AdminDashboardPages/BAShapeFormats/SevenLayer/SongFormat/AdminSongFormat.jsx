...
import EditGoodSongFormatModal from "./EditGoodSongFormatModal";
import SongFormatModal from "./SongFormatModal";

const AdminSongFormat = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) =>
      axiosPublic.put(`/seven-layer/goodSong/${id}`, data),
    onSuccess: () => {
  ...
  const { data: goodSongField = [], isLoading: developLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/seven-layer/goodSongField");
  ...
  const createMutation = useMutation({
    mutationFn: (newData) =>
      axiosPublic.post("/seven-layer/goodSong", newData),
    onSuccess: () => {
      queryClient.invalidateQueries(["goodSong"]);
      Swal.fire(
        "✅ Success!",
  ...
  const { data: goodSong = [], isLoading } = useQuery({
    queryKey: ["goodSong"],
    queryFn: async () => {
      const res = await axiosPublic.get("/seven-layer/goodSong");
      return res.data || [];
    },
  });
  // ✅ Delete Song
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/seven-layer/goodSong/${id}`),
    onSuccess: (res) => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(`/seven-layer/goodSongField/toggle`, {
        fieldName: "isActive",
...
export default AdminSongFormat;
