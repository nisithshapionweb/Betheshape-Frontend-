...
import GoodLifeStyleFormatModal from "./GoodLifeStyleFormatModal";

const AdminGoodLifeStyleFormat = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/goodLifeStyleField");
  ...
  const createMutation = useMutation({
    mutationFn: (newData) =>
      axiosPublic.post("/six-layer/goodLifeStyle", newData),
    onSuccess: () => {
  ...
  const { data: goodLifeStyles = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/goodLifeStyle");
      return res.data || [];
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) =>
      axiosPublic.put(`/six-layer/goodLifeStyle/${id}`, data),
    onSuccess: () => {
      Swal.fire(
        "✅ Updated!",
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/six-layer/goodLifeStyle/${id}`),
    onSuccess: (res) => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(
        `/six-layer/goodLifeStyleField/toggle`,
        {
          fieldName: "isActive",
...
export default AdminGoodLifeStyleFormat;
