...
import CorporateEmailFormatModal from "./CorporateEmailFormatModal";

const AdminCorporateEmailFormat = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/corporateEmailField");
  ...
  const createMutation = useMutation({
    mutationFn: (newData) =>
      axiosPublic.post("/six-layer/corporateEmail", newData),
    onSuccess: () => {
  ...
  const { data: corporateEmails = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/corporateEmail");
      return res.data || [];
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) =>
      axiosPublic.put(`/six-layer/corporateEmail/${id}`, data),
    onSuccess: () => {
      Swal.fire(
        "✅ Updated!",
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/six-layer/corporateEmail/${id}`),
    onSuccess: (res) => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(
        `/six-layer/corporateEmailField/toggle`,
        {
          fieldName: "isActive",
...
export default AdminCorporateEmailFormat;
