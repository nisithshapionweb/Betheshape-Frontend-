...
const DevelopYourSkillsFormatExcersice = () => {
  const axiosPublic = useAxiosPublic();
  const queryClient = useQueryClient();
  const [showAll, setShowAll] = useState(false);

  // 🔹 GET all Develop Your Skills Exercises
  const {
    data: exercises = [],
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["exercise-develop-skills"],
    queryFn: async () => {
      const res = await axiosPublic.get(
        "/six-layer/getAllExerciseDevelopSkills",
      );
  ...
  const { mutate: deleteExercise } = useMutation({
    mutationFn: (id) =>
      axiosPublic.delete(`/six-layer/deleteExerciseDevelopSkills/${id}`),
    onSuccess: () => {
      Swal.fire(
        "Deleted!",
        "Develop Your Skills exercise deleted successfully",
        "success",
      );
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: "Are you sure?",
      text: "This Develop Your Skills exercise will be permanently deleted!",
      icon: "warning",
  ...
  if (isError) {
    return (
      <p className="text-center text-red-500 py-10">
        Failed to load Develop Your Skills exercises
      </p>
    );
  }
    ...
    <div className="max-w-[1400px] mx-auto space-y-6">
      ...
      {!isLoading && exercises.length === 0 && (
        <div className="flex justify-center items-center py-20">
          ...
          >
            🚫 No Develop Your Skills Exercise Found
          </button>
        </div>
      )}
...
export default DevelopYourSkillsFormatExcersice;
