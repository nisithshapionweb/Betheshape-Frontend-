...
const CorporateEmailFormatExercise = () => {
  ...
  const {
    ...
    queryKey: ["exercise-corporate-email"],
    queryFn: async () => {
      const res = await axiosPublic.get(
        "/six-layer/getAllExerciseCorporateEmail"
      );
  ...
  const { mutate: deleteExercise } = useMutation({
    mutationFn: (id) =>
      axiosPublic.delete(
        `/six-layer/deleteExerciseCorporateEmail/${id}`
      ),
    onSuccess: () => {
      Swal.fire(
        "Deleted!",
        "Corporate email exercise deleted successfully",
        "success"
      );
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: "Are you sure?",
      text: "This corporate email exercise will be permanently deleted!",
      icon: "warning",
  ...
  if (isError) {
    return (
      <p className="text-center text-red-500 py-10">
        Failed to load corporate email exercises
      </p>
    );
  }
    ...
    <div className="max-w-[1400px] mx-auto space-y-6">
      ...
      {!isLoading && exercises.length === 0 && (
        <div className="flex justify-center items-center py-20">
          ...
          >
            🚫 No Corporate Email Exercise Found
          </button>
        </div>
      )}
...
export default CorporateEmailFormatExercise;
