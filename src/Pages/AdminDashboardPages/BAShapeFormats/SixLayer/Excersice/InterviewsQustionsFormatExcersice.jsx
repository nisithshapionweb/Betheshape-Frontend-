...
const InterviewsQustionsFormatExcersice = () => {
  const axiosPublic = useAxiosPublic();
  const queryClient = useQueryClient();
  const [showAll, setShowAll] = useState(false);

  // 🔹 GET all Interview Questions Exercises
  const {
    data: exercises = [],
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["exercise-interview-Questions"],
    queryFn: async () => {
      const res = await axiosPublic.get(
        "/six-layer/getAllExerciseInterviewQuestions"
      );
  ...
  const { mutate: deleteExercise } = useMutation({
    mutationFn: (id) =>
      axiosPublic.delete(
        `/six-layer/deleteExerciseInterviewQuestions/${id}`
      ),
    onSuccess: () => {
      Swal.fire(
        "Deleted!",
        "Interview Questions exercise deleted successfully",
        "success"
      );
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: "Are you sure?",
      text: "This Interview Questions exercise will be permanently deleted!",
      icon: "warning",
  ...
  if (isError) {
    return (
      <p className="text-center text-red-500 py-10">
        Failed to load Interview Questions exercises
      </p>
    );
  }
    ...
    <div className="max-w-[1400px] mx-auto space-y-6">
      ...
      {!isLoading && exercises.length === 0 && (
        <div className="flex justify-center items-center py-20">
          ...
          >
            🚫 No Interview Questions Exercise Found
          </button>
        </div>
      )}
...
export default InterviewsQustionsFormatExcersice;
