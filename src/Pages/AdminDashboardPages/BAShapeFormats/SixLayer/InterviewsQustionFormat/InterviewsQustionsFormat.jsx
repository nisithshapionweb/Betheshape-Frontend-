...
import InterviewsQustionsFormatModal from "./InterviewsQustionsFormatModal";


const InterviewsQustionsFormat = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/interviewQuestionsField");
  ...
  const createMutation = useMutation({
    mutationFn: (newData) =>
      axiosPublic.post("/six-layer/interviewQuestions", newData),
    onSuccess: () => {
  ...
  const { data: interviewQuestionss = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/interviewQuestions");
      return res.data || [];
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) =>
      axiosPublic.put(`/six-layer/interviewQuestions/${id}`, data),
    onSuccess: () => {
      Swal.fire(
        "✅ Updated!",
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/six-layer/interviewQuestions/${id}`),
    onSuccess: (res) => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(
        `/six-layer/interviewQuestionsField/toggle`,
        {
          fieldName: "isActive",
...
export default InterviewsQustionsFormat;
