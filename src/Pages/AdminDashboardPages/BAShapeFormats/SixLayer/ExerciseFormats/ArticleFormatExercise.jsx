...
const ArticleFormatExercise = () => {
  const axiosPublic = useAxiosPublic();
  const queryClient = useQueryClient();
  const [showAll, setShowAll] = useState(false);

  // 🔹 GET exercise Article (grouped by user submission)
  const {
    data: exercises = [],
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["exercise-Article"],
    queryFn: async () => {
      const res = await axiosPublic.get(
        "/six-layer/getAllExerciseArticle",
      );
  ...
  const { mutateAsync: deleteExercise } = useMutation({
    mutationFn: async (id) =>
      axiosPublic.delete(`/six-layer/deleteExerciseArticle/${id}`),
    onSuccess: () => {
      Swal.fire("Deleted!", "Exercise deleted successfully", "success");
      queryClient.invalidateQueries({ queryKey: ["exercise-Article"] });
    },
    onError: (error) => {
      Swal.fire(
        "Error",
  ...
  if (isError)
    return (
      <p className="text-center text-red-500 py-10">
        Failed to load exercise Article
      </p>
    );
...
export default ArticleFormatExercise;
