...
const TenseFormatExercise = () => {
  const axiosPublic = useAxiosPublic();
  const queryClient = useQueryClient();
  const [showAll, setShowAll] = useState(false);

  // 🔹 GET exercise Tense (grouped by user submission)
  const {
    data: exercises = [],
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["exercise-Tense"],
    queryFn: async () => {
      const res = await axiosPublic.get(
        "/six-layer/getAllExerciseTense",
      );
  ...
  const { mutateAsync: deleteExercise } = useMutation({
    mutationFn: async (id) =>
      axiosPublic.delete(`/six-layer/deleteExerciseTense/${id}`),
    onSuccess: () => {
      Swal.fire("Deleted!", "Exercise deleted successfully", "success");
      queryClient.invalidateQueries({ queryKey: ["exercise-Tense"] });
    },
    onError: (error) => {
      Swal.fire(
        "Error",
  ...
  return (
    <div className="space-y-10">
      {visibleExercises.length > 0 ? (
        ...
        <p className="text-center py-10 text-gray-500">No Tense found.</p>
      )}
...
export default TenseFormatExercise;
