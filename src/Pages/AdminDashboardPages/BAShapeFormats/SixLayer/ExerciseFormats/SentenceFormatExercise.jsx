...
const SentenceFormatExercise = () => {
  const axiosPublic = useAxiosPublic();
  const queryClient = useQueryClient();
  const [showAll, setShowAll] = useState(false);

  // 🔹 GET exercise Sentence (grouped by user submission)
  const {
    data: exercises = [],
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["exercise-Sentence"],
    queryFn: async () => {
      const res = await axiosPublic.get(
        "/six-layer/getAllExerciseSentence",
      );
  ...
  const { mutateAsync: deleteExercise } = useMutation({
    mutationFn: async (id) =>
      axiosPublic.delete(`/six-layer/deleteExerciseSentence/${id}`),
    onSuccess: () => {
      Swal.fire("Deleted!", "Exercise deleted successfully", "success");
      queryClient.invalidateQueries({ queryKey: ["exercise-Sentence"] });
    },
    onError: (error) => {
      Swal.fire(
        "Error",
  ...
  if (isError)
    return (
      <p className="text-center text-red-500 py-10">
        Failed to load exercise Sentence
      </p>
    );
  ...
  return (
    <div className="space-y-10">
      {visibleExercises.length > 0 ? (
        ...
        <p className="text-center py-10 text-gray-500">No Sentence found.</p>
      )}
...
export default SentenceFormatExercise;
