...
import EditIdeaShareFormatModal from "./EditIdeaShareFormatModal";
import IdeaShareAnsSuggestionFormatModal from "./IdeaShareAnsSuggestionFormatModal";

const IdeaShareAnsSuggestionFormat = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) =>
      axiosPublic.put(`/six-layer/ideaShares/${id}`, data),
    onSuccess: () => {
  ...
  const { data: ideashareFields = [], isLoading: developLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/ideaSharesField");
  ...
  const createMutation = useMutation({
    mutationFn: (newData) =>
      axiosPublic.post("/six-layer/ideaShares", newData),
    onSuccess: () => {
      queryClient.invalidateQueries(["ideashares"]);
      Swal.fire(
        "✅ Success!",
  ...
  const { data: ideashares = [], isLoading } = useQuery({
    queryKey: ["ideashares"],
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/ideaShares");
      return res.data || [];
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/six-layer/ideaShares/${id}`),
    onSuccess: (res) => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(`/six-layer/ideaSharesField/toggle`, {
        fieldName: "isActive",
  ...
  return (
    <>
      ...
      {/* ✅ Modal */}
      {modalOpen && (
        <IdeaShareAnsSuggestionFormatModal
          isOpen={modalOpen}
...
export default IdeaShareAnsSuggestionFormat;
