...
import BeforeProfessionalFormatModal from "./BeforeProfessionalFormatModal";


const AdminBeforeProfessionalFormat = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/beforeProfessionalField");
  ...
  const createMutation = useMutation({
    mutationFn: (newData) =>
      axiosPublic.post("/six-layer/beforeProfessional", newData),
    onSuccess: () => {
  ...
  const { data: beforeProfessionals = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosPublic.get("/six-layer/beforeProfessional");
      return res.data || [];
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) =>
      axiosPublic.put(`/six-layer/beforeProfessional/${id}`, data),
    onSuccess: () => {
      Swal.fire(
        "✅ Updated!",
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axiosPublic.delete(`/six-layer/beforeProfessional/${id}`),
    onSuccess: (res) => {
  ...
  const toggleIsActiveMutation = useMutation({
    mutationFn: async (currentState) => {
      const res = await axiosPublic.put(
        `/six-layer/beforeProfessionalField/toggle`,
        {
          fieldName: "isActive",
...
export default AdminBeforeProfessionalFormat;
