...
const AdminElegantFormatEdit = () => {
  ...
  const { data: elegant, isLoading } = useQuery({
    queryKey: ["elegant", id],
    queryFn: async () => {
      const res = await axiosPublic.get(`/six-layer/elegant/${id}`);
      return res.data.data;
    },
  });
 ...
 const onSubmit = async (data) => {
  try {
    await axiosPublic.put(`/six-layer/elegant/${id}`, data);
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-2">
      <Helmet>
        <title>Edit Elegant Format</title>
      </Helmet>
...
export default AdminElegantFormatEdit;
