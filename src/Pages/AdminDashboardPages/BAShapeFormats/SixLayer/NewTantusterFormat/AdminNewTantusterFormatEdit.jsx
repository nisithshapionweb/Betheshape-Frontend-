...
const AdminNewTantusterFormatEdit = () => {
  const { id } = useParams();
  ...
  const { data: newtantuster, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosPublic.get(`/six-layer/newtantuster/${id}`);
      return res.data.data;
    },
  });
 ...
 const onSubmit = async (data) => {
  try {
    await axiosPublic.put(`/six-layer/newtantuster/${id}`, data);
...
export default AdminNewTantusterFormatEdit;
